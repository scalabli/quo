...
_T = ty.TypeVar("_T", bound=OneStyleAndTextTuple)


class _ExplodedList(ty.List[_T]):
    """
    ...
    def extend(self, lst: ty.Iterable[_T]) -> None:
    ...
    @ty.overload
    def __setitem__(self, index: slice, value: ty.Iterable[_T]) -> None:
        ...

    def __setitem__(
        self, index: ty.Union["SupportsIndex", slice], value: ty.Union[_T, ty.Iterable[_T]]
    ) -> None:
        """
        ...
        if isinstance(value, tuple):  # In case of `OneStyleAndTextTuple`.
            value = ty.cast("ty.List[_T]", [value])

        super().__setitem__(index, explode_text_fragments(ty.cast("ty.Iterable[_T]", value)))


def explode_text_fragments(fragments: ty.Iterable[_T]) -> _ExplodedList[_T]:
    """
...
"""
Decoy Data structures for the Buffer to avoid circular imports.
...
from quo.i_o.util import Completer,Completion, CompleteEvent
...
from quo.search import SearchDirection, SearchState
from quo.selection import PasteMode, SelectionState, SelectionType
from quo.utils import Event, to_str
