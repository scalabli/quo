...
from .formatting import join_apps
...
def _complete_visible_commands(
    clime: "Clime", incomplete: str
) -> t.Iterator[t.Tuple[str, "Command"]]:
    """List all the subcommands of a group that start with the
    incomplete value and aren't hidden.

    :param clime: Invocation context for the tether.
    :param incomplete: Value being completed. May be empty.
    """
    multi = t.cast(MultiCommand, clime.command)

    for name in multi.list_commands(clime):
        if name.startswith(incomplete):
            command = multi.get_command(clime, name)
...
def augment_usage_errors(
    clime: "Clime", param: t.Optional["Parameter"] = None
) -> t.Iterator[None]:
...
class ParameterSource(enum.Enum):
    ...
    Use :meth:`quo.Clime.get_parameter_source` to get the
...
class Clime:
    ...
    :param ignore_unknown_apps: instructs Quo to ignore apps it does
                                   ...
                                   processing.
    :param help_app_names: optionally a list of strings that define how
    ...
    .. versionchanged:: 4.0
        Added the ``color``, ``ignore_unknown_apps``, and
    ...
    .. versionchanged:: 2.0
        Added the ``resilient_parsing``, ``help_app_names``, and
    ...
    def __init__(
        ...
        ignore_unknown_apps: t.Optional[bool] = None,
        help_app_names: t.Optional[t.List[str]] = None,
        ...
        #:
        #: .. versionadded:: 4.0
        self.ignore_unknown_apps: bool = ignore_unknown_apps

        if help_app_names is None:
            if parent is not None:
                help_app_names = parent.help_app_names
            else:
                help_app_names = ["--help"]

        #: The names for the help apps.
        self.help_app_names: t.List[str] = help_app_names
    ...
    @contextmanager
    def scope(self, cleanup: bool = True) -> t.Iterator["Clime"]:
        ...
        Example usage::
            ...
            with clime.scope():
                assert get_current_context() is clime
    ...
    def _make_sub_context(self, command: "Command") -> "Clime":
    ...
    def invoke(
        ...
        **kwargs: t.Any,
        ...
        if isinstance(__callback, Command):
            ...
            clime = __self._make_sub_context(other_cmd)

            for param in other_cmd.params:
                if param.name not in kwargs and param.expose_value:
                    kwargs[param.name] = param.type_cast_value(  # type: ignore
                        clime, param.get_default(clime)
                    )

            # Track all kwargs as params, so that forward() will pass
            # them on in subsequent calls.
            clime.params.update(kwargs)
        else:
            clime = __self
...
class BaseCommand:
    ...
    #:
    #: .. versionadded:: 8.0
    context_class: t.Type[Clime] = Clime
    ...
    def __init__(
        ...
        #: on a :class:`Group` the tether will default the command name
    ...
    def to_info_dict(self, clime: Clime) -> t.Dict[str, t.Any]:
        """Gather information that could be useful for a tool generating
        user-facing documentation. This traverses the entire structure
        below this command.

        Use :meth:`quo.Clime.to_info_dict` to traverse the entire
        CLI structure.
    ...
    def get_usage(self, clime: Clime) -> str:
        raise NotImplementedError("Base commands cannot get usage")

    def get_help(self, clime: Clime) -> str:
    ...
    def make_context(
        ...
        **extra: t.Any,
        ...
        off the parsing and create a new :class:`Clime`.  It does not
    ...
    def parse_args(self, clime: Clime, args: t.List[str]) -> t.List[str]:
    ...
    def invoke(self, clime: Clime) -> t.Any:
        """Given a context, this invokes the command.  The default
        implementation is raising a not implemented error.
        """
        raise NotImplementedError("Base commands are not invokable by default")

    def shell_complete(self, clime: Clime, incomplete: str) -> t.List["CompletionItem"]:
        ...
        .. versionadded:: 8.0
        """
        from quo.shell_completion import CompletionItem
    ...
    def main(
        ...
        **extra: t.Any,
        ...
        :param extra: extra keyword arguments are forwarded to the context
                      constructor.  See :class:`Clime` for more information.
...
class Command(BaseCommand):
    ...
    :param add_help_app: by default each command registers a ``--help``
                            option.  This can be disabled by this parameter.
    :param no_args_is_help: this controls what happens if no args are
    ...
    def __init__(
        ...
        apps_metavar: t.Optional[str] = "[OPTIONS]",
        add_help_app: bool = True,
        ...
        self.add_help_app = add_help_app
    ...
    def get_params(self, clime: Clime) -> t.List["Parameter"]:
        rv = self.params
        help_app = self.get_help_app(clime)
    ...
    def format_usage(self, clime: Clime, formatter: HelpFormatter) -> None:
    ...
    def collect_usage_pieces(self, clime: Clime) -> t.List[str]:
    ...
    def get_help_app_names(self, clime: Clime) -> t.List[str]:
        """Returns the names for the help option."""
        all_names = set(clime.help_app_names)
        for param in self.params:
            all_names.difference_update(param.opts)
            all_names.difference_update(param.secondary_opts)
        return list(all_names)

    def get_help_app(self, clime: Clime) -> t.Optional["Option"]:
        """Returns the help option object."""
        help_apps = self.get_help_app_names(clime)

        if not help_apps or not self.add_help_app:
            return None

        def show_help(clime: Clime, param: "Parameter", value: str) -> None:
    ...
    def make_parser(self, clime: Clime) -> OptionParser:
        """Creates the underlying option parser for this command."""
        parser = OptionParser(clime)
        for param in self.get_params(clime):
    ...
    def format_help(self, clime: Clime, formatter: HelpFormatter) -> None:
    ...
    def format_help_text(self, clime: Clime, formatter: HelpFormatter) -> None:
    ...
    def format_apps(self, clime: Clime, formatter: HelpFormatter) -> None:
        ...
        if opts:
            with formatter.section(_("Apps")):
                formatter.write_dl(opts)

    def format_epilog(self, clime: Clime, formatter: HelpFormatter) -> None:
        ...
        if incomplete and not incomplete[0].isalnum():
            for param in self.get_params(clime):
                if (
                    ...
                    or (
                        not param.multiple
                        and clime.get_parameter_source(param.name)  # type: ignore
...
class MultiCommand(Command):
    ...
    def __init__(
        ...
        **attrs: t.Any,
        ...
        if self.chain:
            for param in self.params:
                if isinstance(param, Arg) and not param.required:
                    raise RuntimeError(
    ...
    def format_commands(self, clime: Clime, formatter: HelpFormatter) -> None:
    ...
    def invoke(self, clime: Clime) -> t.Any:
        def _process_result(value: t.Any) -> t.Any:
            if self._result_callback is not None:
                value = clime.invoke(self._result_callback, value, **clime.params)
            return value

        if not clime.protected_args:
            if self.invoke_without_command:
                # No subcommand was invoked, so the result callback is
                # invoked with None for regular tethers, or an empty list
                # for chained tethers.
                with clime:
                    super().invoke(clime)
                    return _process_result([] if self.chain else None)
            clime.fail(_("Missing command."))
    ...
    def resolve_command(
        self, clime: Clime, args: t.List[str]
        ...
        if cmd is None and not clime.resilient_parsing:
            ...
            clime.fail(_("No such command {name!r}.").format(name=original_cmd_name))
        return cmd_name if cmd else None, cmd, args[1:]

    def get_command(self, clime: Clime, cmd_name: str) -> t.Optional[Command]:
        """Given a context and a command name, this returns a
        :class:`Command` object if it exists or returns `None`.
        """
        raise NotImplementedError

    def list_commands(self, clime: Clime) -> t.List[str]:
...
class Tether(MultiCommand):
    ...
    :param name: The name of the tether command.
...
class Parameter:
    ...
    :param callback: A function to further process or validate the value
        after type conversion. It is called as ``f(clime, param, value)``
    ...
    :param shell_complete: A function that returns custom shell
        ...
        of :class:`~quo.shell_completion.CompletionItem` or a list of
        strings.
    ...
    def __init__(
        ...
        callback: t.Optional[t.Callable[[Clime, "Parameter", t.Any], t.Any]] = None,
    ...
    @t.overload
    def get_default(
        self, clime: Clime, call: "te.Literal[True]" = True
    ) -> t.Optional[t.Any]:
        ...

    @t.overload
    def get_default(
        self, clime: Clime, call: bool = ...
    ) -> t.Optional[t.Union[t.Any, t.Callable[[], t.Any]]]:
        ...

    def get_default(
        self, clime: Clime, call: bool = True
    ) -> t.Optional[t.Union[t.Any, t.Callable[[], t.Any]]]:
        """Get the default for the parameter. Tries
        :meth:`Clime.lookup_default` first, then the local default.
        ...
        .. versionchanged:: 8.0
            Added the ``call`` parameter.
        """
        value = clime.lookup_default(self.name, call=False)  # type: ignore
    ...
    def add_to_parser(self, parser: OptionParser, clime: Clime) -> None:
        raise NotImplementedError()

    def consume_value(
        self, clime: Clime, opts: t.Mapping[str, t.Any]
        ...
        if value is None:
            value = clime.lookup_default(self.name)  # type: ignore
    ...
    def type_cast_value(self, clime: Clime, value: t.Any) -> t.Any:
        ...
        def check_iter(value: t.Any) -> t.Iterator:
            try:
                ...
                # the command line.
                raise BadParameter(
                    _("Value must be an iterable."), clime=clime, param=self
                ) from None

        if self.nargs == 1 or self.type.is_composite:
            convert: t.Callable[[t.Any], t.Any] = partial(
                self.type, param=self, clime=clime
            )
        elif self.nargs == -1:

            def convert(value: t.Any) -> t.Tuple:
                return tuple(self.type(x, self, clime) for x in check_iter(value))
    ...
    def process_value(self, clime: Clime, value: t.Any) -> t.Any:
        value = self.type_cast_value(clime, value)
    ...
    def resolve_envvar_value(self, clime: Clime) -> t.Optional[str]:
    ...
    def value_from_envvar(self, clime: Clime) -> t.Optional[t.Any]:
        rv: t.Optional[t.Any] = self.resolve_envvar_value(clime)
    ...
    def handle_parse_result(
        self, clime: Clime, opts: t.Mapping[str, t.Any], args: t.List[str]
    ) -> t.Tuple[t.Any, t.List[str]]:
        with augment_usage_errors(clime, param=self):
            value, source = self.consume_value(clime, opts)
            clime.set_parameter_source(self.name, source)  # type: ignore
        ...
        if self.expose_value:
            clime.params[self.name] = value  # type: ignore

        return value, args

    def get_help_record(self, clime: Clime) -> t.Optional[t.Tuple[str, str]]:
        pass

    def get_usage_pieces(self, clime: Clime) -> t.List[str]:
        return []

    def get_error_hint(self, clime: Clime) -> str:
...
class App(Parameter):
    ...
    :param show_default: Controls if the default value should be shown
        ...
        apps. For single option boolean flags, the default remains
    ...
    :param flag_value: which value should be used for this flag if it's
                       ...
                       the option string contains a slash to mark two apps.
    ...
    def __init__(
        ...
        **attrs: t.Any,
        ...
        if __debug__:
            if self.nargs == -1:
                raise TypeError("nargs=-1 is not supported for apps.")
    ...
    def get_help_record(self, clime: Clime) -> t.Optional[t.Tuple[str, str]]:
        ...
        if self.show_envvar:
            ...
            if envvar is None:
                if (
                    ...
                    and clime.auto_envvar_prefix is not None
        ...
        resilient = clime.resilient_parsing
        clime.resilient_parsing = True

        try:
            default_value = self.get_default(clime, call=False)
        finally:
            clime.resilient_parsing = resilient

        show_default_is_str = isinstance(self.show_default, str)

        if show_default_is_str or (
            default_value is not None and (self.show_default or clime.show_default)
        ):
    ...
    def prompt_for_value(self, clime: Clime) -> t.Any:
    ...
    def consume_value(
        self, clime: Clime, opts: t.Mapping[str, "Parameter"]
    ) -> t.Tuple[t.Any, ParameterSource]:
