...
from quo.eventloop.utils import call_soon_threadsafe, get_event_loop
...
from quo.i_o.termui import EditingMode
...
from quo.i_o.output import ColorDepth, Output
...
from quo.application.current import get_app_session, set_app
from quo.application.run_in_terminal import in_terminal, run_in_terminal
...
ApplicationEventHandler = ty.Callable[["Application[_AppResult]"], None]
...
class Suite(ty.Generic[_AppResult]):
    """
    The main Suite class decoy class to prevent circular imports. 
    ...
    def __init__(
        ...
        terminal_size_polling_interval: ty.Optional[float] = 0.5,
        on_reset: ty.Optional["ApplicationEventHandler[_AppResult]"] = None,
        on_invalidate: ty.Optional["ApplicationEventHandler[_AppResult]"] = None,
        before_render: ty.Optional["ApplicationEventHandler[_AppResult]"] = None,
        after_render: ty.Optional["ApplicationEventHandler[_AppResult]"] = None,
        # I/O.
        ...
        # ty.List of 'extra' functions to execute before a Suite.run.
        ...
        self._invalidate_events: ty.List[
            Event[object]
 ...
 #       )  # Dummy buffer.
    ...
    def reset(self) -> None:
        ...
        self.background_tasks: ty.List[Task[None]] = []
        ...
        if self.min_redraw_interval:
            ...
            if diff < self.min_redraw_interval:
                ...
                async def redraw_in_future() -> None:
                    await sleep(ty.cast(float, self.min_redraw_interval) - diff)
                    schedule_redraw()
    ...
    def _redraw(self, render_as_done: bool = False) -> None:
        ...
        #       Suite is not the active one. (Like the
    ...
    def _update_invalidate_events(self) -> None:
        ...
        def gather_events() -> ty.Iterable[Event[object]]:
    ...
    async def run_async(
        ...
        async def _run_async() -> _AppResult:
            ...
            with self.input.raw_mode(), self.input.attach(
                ...
                return ty.cast(_AppResult, result)
    ...
    @contextmanager
    def _enable_breakpointhook(self) -> ty.Generator[None, None, None]:
        """
    ...
    def _breakpointhook(self, *a: object, **kw: object) -> None:
        ...
        TraceDispatch = ty.Callable[[FrameType, str, Any], Any]
    ...
    @ty.overload
    def exit(
        self, *, exception: ty.Union[BaseException, ty.Type[BaseException]], style: str = ""
    ) -> None:
        "Exit with exception."
    ...
    def exit(
        ...
        exception: ty.Optional[ty.Union[BaseException, ty.Type[BaseException]]] = None,
        style: str = "",
    ) -> None:
        """
        Exit application.
        ...
        if exception is not None:
            ...
            self.future.set_result(ty.cast(_AppResult, result))
...
class _CombinedRegistry(KeyBindingsBase):
    """
    The `KeyBinder` of key bindings for a `Suite`.
...
async def _do_wait_for_enter(wait_text: str) -> None:
    """
...
def attach_winch_signal_handler(
    ...
    # loop with a wakeup fd, but another Suite could have registered
