...
Prints a message plus a newline to the given file or stdout. On first sight, this looks like the print function, but it has improved support for handling Unicode and binary data that does not fail no matter how badl configured the system is.

Primarily it means that you can print binary data as well as Unicode data to the given file in the most appropriate way possible. If colorama is installed, the echo function will also support clever handling of ANSI color codes.
...
The context manager creates the progress bar. When the context manager is entered the progress bar is already created. With every iteration over the progress bar, the iterable passed to the bar is advanced and the bar is updated. When the context manager exits, a newline is printed and the progress bar is finalized
...
color – controls if the terminal supports ANSI colors or not. The default is autodetection. This is only needed if ANSI codes are included anywhere in the progress bar output which is not the case 
...
Styles a text with ANSI styles  the styling is self contained which means that at the end of the string a rese

Examples:

click.echo(quo.style('Hello World!', fg='green'))
quo.echo(quo.style('ATTENTION!', blink=True))
quo.echo(quo.style('Some things
Supported color names:
...
Parameters
   ...
   * ``extension`` – the extension to tell the editor about. This defaults to .txt

   * ``filename`` – if provided it will edit this file instead of the provided text contents.
...
.. code:: python
   ...
   launch('https://quo.palletsprojects.com/')
   launch('/my/downloaded/file', locate=True)
...
Parameters
   ...
   * ``locate`` – if this is set to True then instead of launching the application associated with the URL it will attempt to launch a file manager with the file located.

**quo.interpose** *(inscribe=False)*
...
Changelog
Parameters
   * ``inscribe`` – if set to True, the character read will also show up on the terminal. The default is to not show it.

**quo.pause** *(info='Press any key to continue ...', err=False)**
This command stops execution and waits for the user to press any key to continue. If the program is not run through a terminal, this command will instead do nothing.
...
**quo.textstream** *(name, encoding=None, errors='strict')*
Returns a system stream for text processing. This usually returns a wrapped stream around a binary stream returned from binarystream() but it also can take shortcuts on Python 3 for already correctly configured streams.
...
**quo.openfile** *(filename, mode='r', encoding=None, errors='strict', lazy=False, atomic=False)*
...
**quo.appdir** *(app_name, roaming=True, force_posix=False)*
...
**quo.formatfilename** *(filename, shorten=False)*
Formats a filename for user display. The main purpose of this function is to ensure that the filename can be displayed. This will decode the filename to unicode if necessary in a way that it will not fail. Optionally, it can shorten the filename to not include the full path to the filename.

Parameters
   * ``filename`` – formats a filename for display. This will also convert the filename into unicode.
...
.. autofunction:: binarystream

.. autofunction:: textstream

.. autofunction:: openfile

.. autofunction:: appdir

.. autofunction:: formatfilename
